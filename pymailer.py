...
"""
pymailer.py is the command line interface for pyMailer.
...
Options:
    ...
    -s,--subject Subject
    -f,--from    Fromaddr
    --smtpstr    Smtp string in the format:
...
def Mail(sender, rcpts, smtpStr='|||', subject=None, body=None,
         fileList=[], sendinline=0):
  """
  Sends mail.
  Args:
    sender: sender's email address
    rcpts: list of receipients
    smtpStr: smtp options string in the format -
             'server:port|encrypt|user|password'
    subject: subject
    body: message body
    fileList: a list of files to be sent as an attachment
    sendinline: if set to 1, files are sent inline in message body.
  """
  msg = MIMEMultipart()
  msg['Subject'] = subject
  msg['To'] = ', '.join(rcpts)
  ...
  for filename in fileList:
    ...
    if sendinline:
      msgpart.add_header('Content-Disposition', 'inline',
          filename=os.path.basename(filename))
    else:
      msgpart.add_header('Content-Disposition', 'attachment',
          filename=os.path.basename(filename))
    msg.attach(msgpart)
...
def main():
  """
  This function does the command line parsing and calls Mail.
  """
  import getopt
  attachList=[]
  try:
  ...
  if smtpsvr is not '' and smtpstr is '|||':
      smtpstr=smtpsvr+'|||'
  ret,msg=Mail(sender=fromaddr,rcpts=args,subject=sub,
               body=body,fileList=attachList,smtpStr=smtpstr)
  if ret == 1:
